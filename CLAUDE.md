...
**Common `verbosity` param**: the core analysis tools accept `verbosity` (`"full"` default,
or `"brief"`). `_apply_verbosity()` strips the echoed-back input (and the DREAD worked
examples) when `"brief"` is requested, to save tokens for agents driving many calls; `"full"`
is unchanged for backward compatibility. Preserve this default-`full` behaviour when editing
tool handlers.

## Resources (companion skill served over MCP)

Alongside the tools, the server exposes the `stride-threat-modelling` skill's files over the
MCP **resources** interface so MCP-only clients (no Agent Skills support) can still pull the
canonical guidance:

- Advertised via the `resources` capability in `initialize` / `server/discover`.
- `resources/list` enumerates the skill files; `resources/read` returns one by `uri`.
- URIs are `stride://skill/<relative-path>` (e.g. `stride://skill/references/report-format.md`).
- `_resolve_skill_uri()` is path-traversal safe and `_skill_files()` degrades to `[]` if the
  skill dir isn't bundled — keep both invariants when touching this code.
- The skill files are bundled into the Vercel function via `includeFiles` in `vercel.json`;
  if the skill grows new files, make sure they're still shipped.
